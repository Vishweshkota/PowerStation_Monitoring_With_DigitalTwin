...
public class HumidityAnimationHandler: MonoBehaviour, IDataContextEventListener
{
    
    [SerializeField, Range(60.0f, 100.0f)]
    private float thresholdHigh = 90.0f;

    [SerializeField, Range(30.0f, 60.0f)]
    private float nominalMid = 50.0f;

    [SerializeField, Range(1.0f, 30.0f)]
    private float thresholdLow = 20.0f;

    private Renderer humidityObjectRenderer = null;
    private Gradient humidityGradient = null; 

     void Start()
    {
        // get renderer and create gradient
        this.humidityObjectRenderer = gameObject.GetComponent<Renderer>();
        this.humidityGradient = new Gradient();
        ...
        this.humidityGradient.SetKeys(colorKey, alphaKey);
    ...
    {
        if (this.humidityObjectRenderer != null)
        {
            ...
            this.humidityObjectRenderer.material.color = this.humidityGradient.Evaluate(scaledVal);
        }
    }
}
